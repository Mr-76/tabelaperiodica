...
def escrever_arquivo(Nome_Arquivo, dicionario_volta):
    with open(Nome_Arquivo, 'w') as arquivo_deTexto:
        # bom para quando chamar dicionario em                                                                 outro programa
        nome = Nome_Arquivo.replace('.py', '').lower()
...
# transformando a lista de tuplas objeto do dicionario em uma lista 'real'
Dicionario_para_lista = list(elementos_eletro.items())


print(Dicionario_para_lista)

# em caps para nome do arquvio e sem caps                                                   para nome do dicionario
...
for tipo in tipos:
    ...
    try:
        ...
        (adicionar_ou_modificar, repetiçao) = input(
            'deseja adicionar ou modificar? A/M e quantas vezes ex: M 10 ').split()
        ...
        for repetir in range(repetiçao):
            ...
            if adicionar_ou_modificar == "M":
                ...
                for item in lista_Tuplas:  # procurando elemento na lista
                    contador += 1  # index
                    # item  se refere ao primeiro elemento da tupla (key , valor)
                ...
                valor = int(
                    input("valor a modificar susbstituir no elemento? "))
            ...
            if adicionar_ou_modificar == "A":
                ...
                (elementoTupla, valorTupla) = input(
                    "digite o elemento e o valor entre espacos ex: Fe 2 ").split()

                # adiciona a lista o novo elemento
                adiciona_a_lista(elementoTupla, valorTupla)
                ...
                # escreve dicionario de volta no arquivo de texto
                escrever_arquivo(Nome_Arquivo, dicionario_volta)
        ...
        for index_tupla in range(len(Dicionario_para_lista)):
            ...
            valor = input(
                "digite o valor a ser modificado no elemento caso queria sair apenas \npressione enter ")
